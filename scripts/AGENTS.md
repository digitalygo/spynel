...
- The smoke test covers the user-bin development installer in an isolated target, canonical `.spynel/config.yaml` initialization without `workspace.state_dir`, fresh-workspace retirement checks, byte-for-byte preservation of legacy retired workflow documents across upgrade, offline agent-readable docs, harness-independent plain CLI status, shared deterministic commands, bounded conversation list/show/resume, and strict inactive-follow-up rejection.
