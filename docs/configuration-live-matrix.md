...
| `speech.elevenlabs_api_key_env` | Live | The next ElevenLabs transcription resolves the new environment reference, and a missing or blank value falls back to local Parakeet for that call | Portable variable-name validation rejects before commit; the key value is never read during save | speech key-reference and fallback tests |
