...
Speech covers Telegram and WhatsApp voice notes and audio files, including push-to-talk and ordinary audio. Video, documents (even audio-named ones), images, and stickers are never transcribed. The default provider is local Parakeet; `speech.provider: elevenlabs` switches the same scope to the ElevenLabs cloud API. The two providers never fall back to each other.

- `enabled`: transcribe incoming voice notes and audio files; enabled by default in new workspaces.
- `provider`: `parakeet` or `elevenlabs`; default `parakeet`. It applies live, and the replacement channel adapter wires the new backend on its next use.
...
- `elevenlabs_api_key_env`: ElevenLabs only. Name of the environment variable that holds the API key; default `ELEVENLABS_API_KEY`. The name must be a portable variable identifier (`[A-Za-z_][A-Za-z0-9_]*`, at most 128 bytes). Spynel reads the variable when a transcription runs and never stores, logs, or displays the key value.
