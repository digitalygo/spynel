...
On **2026-09-25**, the installed `@earendil-works/pi-coding-agent` `0.87.1`
source was inspected on this workstation: `SessionManager` assigns the session
file path when a session is created but defers creating the JSONL file until
the first assistant message persists, and a `set_session_name` before that
flush stays in memory until the same flush writes it. No authenticated model
request was made.
...
| Pi coding agent | `pi --mode rpc` with the user's ordinary Pi resources plus one additive Telegram-only note extension; documented JSONL commands and events; no Spynel resource suppression, approval override, or extension allowlist | **Not established.** Spynel requires a non-empty `--version`, successful `get_state`, and both queue-mode configuration commands. | `@earendil-works/pi-coding-agent` `0.84.1` was installed on Linux arm64. Keyless `PI_OFFLINE=1` RPC negotiation succeeded; synthetic lifecycle, steering, cancellation, model, resume, session-control, and session-naming tests pass. No model request was sent. The installed `0.87.1` source was inspected on 2026-09-25: `SessionManager` defers creating the JSONL file until the first assistant message persists, so a live pre-flush session is named in memory; no authenticated model request was made. | Supported native RPC boundary; authenticated model/tool behavior remains unverified. See the [Pi adapter](../internal/harness/pi.go), [tests](../internal/harness/pi_test.go), [session-control tests](../internal/harness/pi_sessions_test.go), and official [RPC documentation](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/rpc.md). |
...
| Retry and terminal selection | **Tested:** Pi retries a transient provider failure (for example an upstream rate limit or overload) inside one settled run with exponential backoff, bounded by the user's own Pi `retry` settings, which Spynel does not override; `auto_retry_start` and `summarization_retry_scheduled` stay nonterminal status. A settled run with at least one successful assistant message reports only its successful messages, newline-joined, and sets `FinalText` to the last successful message; a failed assistant message streams live but contributes no settled text; the terminal error is emitted only when no assistant message succeeded. **Unverified:** no authenticated provider rate-limit or retry canary. | **Not applicable:** the shared ACP adapter defines no in-run retry event; the `session/prompt` response supplies the terminal result. |
...
| Telegram final-replies note | **Tested:** a Telegram Pi RPC session loads one embedded Spynel extension additively with `--extension`. The extension is materialized as a private `0600` regular file under `.spynel/runtime`, and its `before_agent_start` writes exactly one namespaced `spynel_telegram` section into Pi's structured system prompt, stating that the user sees only final replies, so any clarification or approval request must be one final question that ends the turn and the next user reply continues the same session with no artificial pause state. Because the load is additive, Pi keeps discovering global and trusted-project `SYSTEM.md` and `APPEND_SYSTEM.md` files and its ordinary resources; the TUI and every other harness receive no such note. The note is prompt guidance rather than an enforcement boundary: a user extension that returns a forced prompt can override it, and extension-triggered model runs may bypass `before_agent_start`, so Spynel claims no model-compliance guarantee. | **Not applicable:** the note is Pi-specific. |
| Session controls | **Tested:** `/pi session` reports the durable full session ID and a shell-safe `pi --fork` command without starting Pi; `/pi name <name>` sends RPC `set_session_name` for an existing session under an expected-ID fence, naming a matching live session even before Pi flushes its first JSONL file while a persisted resume still requires the stored regular file, preserves and returns an existing provider name on the automatic only-if-empty path, reads the normalized name back from `get_state`, and never creates or rotates a session; `/pi compact [instructions]` calls RPC `compact` on an existing idle, policy-matching session, accepts at most 4096 Unicode-code-point instructions, and maps bounded `tokensBefore`/`estimatedTokensAfter` counts; `/pi import <full-session-id>` forks exactly one validated direct session file into the Spynel session directory without changing the source. Uses the documented [RPC](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/rpc.md), [session](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/sessions.md), [session format](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/session-format.md), and [compaction](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/compaction.md) interfaces. **Unverified:** no live provider compaction, import, or naming canary. | **Not applicable:** the session-control capabilities are Pi-only; the supervisor returns an explicit unsupported error for other harnesses. |
...
- Pi session controls are evidenced by deterministic fixtures and the
  installed `0.87.1` source observation only, never by an authenticated
  provider run. No authenticated provider run performed a live manual
  compaction, external-session import, or session rename, so provider-side
  summary quality, token accounting, fork behavior, and provider name
  normalization on a real installation remain **unverified**.
...
3. At least once per planned compatibility review while compatibility work is
