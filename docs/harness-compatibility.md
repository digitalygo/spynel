...
- Pi session controls are evidenced by deterministic fixtures only. No
  authenticated provider run performed a live manual compaction,
  external-session import, or session rename, so provider-side summary quality,
  token accounting, fork behavior, and provider name normalization on a real
