...
`spynel update` updates its own installation and restarts every running instance of that installation across workspaces, including primaries, headless servers and secondary TUIs. `/update` in a channel selects the workspace primary's installation and performs the same operation. npm installations use npm; script installations use verified stable GitHub bundles. `spynel update check` or `/update check` only checks versions. `update install` also applies the update. Even when already current, an update request restarts the instances. Saved workspace configuration, histories, and job state remain in place.
