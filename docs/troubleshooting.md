...
- ElevenLabs transcription reads its key from the environment variable named by `speech.elevenlabs_api_key_env` (default `ELEVENLABS_API_KEY`) when each transcription runs. Generated autostart services preserve home and cache paths but do not copy other shell-only exports, so a key that works in an interactive shell can still be missing from a background service; make the variable visible to the running Spynel process. A missing or blank value does not fail that transcription: it falls back to local Parakeet. If the fallback also fails, the visible `[Speech transcription failed; inspect the attached audio manually: ...]` marker appears, and the attachment is still delivered.
- With the default `elevenlabs` provider and a visible key, accepted audio, including permitted group audio, goes to the ElevenLabs API. Only a rate-limit failure retries at most once, and only when `Retry-After` is 60 seconds or less; every other failure surfaces immediately. Check provider status and quota when failures repeat; a missing transport-declared duration also fails closed before upload.
