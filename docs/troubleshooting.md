...
- Local Parakeet speech accepts WAV, FLAC, MP3, and Telegram/WhatsApp Ogg/Opus voice notes. M4A/AAC, WebM, and other formats return an unsupported-format error. First supported use downloads a checksum-pinned model into the operating system's per-user cache unless `speech.model_dir` is configured. This local model path applies only to `speech.provider: parakeet`.
- ElevenLabs transcription reads its key from the environment variable named by `speech.elevenlabs_api_key_env` (default `ELEVENLABS_API_KEY`) when each transcription runs. Generated autostart services preserve home and cache paths but do not copy other shell-only exports, so a key that works in an interactive shell can still be missing from a background service; make the variable visible to the running Spynel process. A missing key appears as a visible `[Speech transcription failed; inspect the attached audio manually: ...]` marker, and the attachment is still delivered.
- Selecting `elevenlabs` sends accepted audio, including permitted group audio, to the ElevenLabs API. Only a rate-limit failure retries at most once, and only when `Retry-After` is 60 seconds or less; every other failure surfaces immediately. Check provider status and quota when failures repeat; a missing transport-declared duration also fails closed before upload.
