...
- Document Pi session controls (`/pi session`, `/pi compact [instructions]`, `/pi import <full-session-id>`), their TUI and private-Telegram-only surface boundary including private topics, the fork rationale for the unlocked Pi JSONL session format, the 4096-code-point compaction bound and bounded token counts, bounded read-only import discovery and source immutability, the downstream-only new-session notice, and the unchanged Pi global-resource, project-trust, read-only-policy, and session-policy behavior consistently across integration, CLI, harness-compatibility, and compiled agent documentation.
- Document harness prompt-history behavior consistently across integration, configuration, architecture, CLI, harness-compatibility, and compiled agent documentation: the current user message is always delivered, zero history limits disable only prior-history seeding, Pi receives only the current entry plus the durable history path once a live or persisted session retains the conversation, and every other harness or uncertain state keeps the bounded seed.
...
- Document Telegram private-chat and forum-topic conversation identity (`TG-<user-id>`, `TG-<user-id>-topic-<thread-id>`, `TG-group-<chat-id>`, and `TG-group-<chat-id>-topic-<thread-id>`), automatic default-on topic routing with base-conversation preservation, topic-aware origin authorization, and the bounded `sendMessage` HTML chunk contract (parsed-visible per-message and per-reply limits, balanced formatting, visible truncation marker, single plain-text entity-parse fallback, and one bounded rate-limit retry) consistently across integration, architecture, CLI, and compiled agent documentation.
