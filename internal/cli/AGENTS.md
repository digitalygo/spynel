...
- Keep `docs` offline and workspace-independent. `spynel notify` requires exactly one of explicit `--origin ORIGIN` or `--recent-authorized`, independently revalidates workspace/origin authorization through the application service, and remains one ordinary delivery command. Recent routing exposes no resolved destination and fails closed on ambiguous authority/activity. Operator positional input may remain compatible.
...
- Poll owner shared state for TUI runtime, title, theme, connection, and notice changes through bounded nonblocking channels, and leave bounded diagnostics off the visual header. Main communication-agent activity arrives through the canonical message-stream dispatch/terminal events consumed by the TUI, never as shared-state counts.
...
- Own the primary-term durable Telegram final-reply worker in server composition: construct it from the private runtime queue directory, the election owner ID, and the owner guard; run it for the whole owner lifetime beside the channels; join it before releasing the primary lease; and keep it independent of any Telegram adapter generation so queued delivery and expiry cleanup continue while Telegram is disabled or disconnected. Every short record mutation stays fenced by the exact ownership term, and provider requests run outside the election lock.
