...
- Validate `speech.provider` against `parakeet|elevenlabs` and `speech.elevenlabs_model_id` against `scribe_v2|scribe_v1` after lowercasing and edge trimming, and validate `speech.elevenlabs_api_key_env` as a portable environment-variable name matching `[A-Za-z_][A-Za-z0-9_]*` with at most 128 bytes. All three apply live and are not secret settings: they name a backend and an environment variable, and the referenced key value is never stored in configuration or exposed in status. Keep `speech.language`, `speech.max_file_mb`, and `speech.max_duration_seconds` provider-neutral and treat `speech.model_dir`, `speech.num_threads`, and `speech.chunk_seconds` as Parakeet-only.
