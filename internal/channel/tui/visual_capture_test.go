...
func visualConfigModel() model {
	...
	value.openScreen(core.Screen{
		...
		Controls: []core.ScreenControl{
			...
			{Key: "workspace.history_max_messages", Label: "context messages", Kind: "text", Value: "50", Description: "Maximum recent messages passed to the harness (0 disables prior-history seeding; the current message is always delivered)"},
			{Key: "workspace.history_char_limit", Label: "context characters", Kind: "text", Value: "12000", Description: "Maximum total history characters passed to the harness; bounds the current message when positive"},
