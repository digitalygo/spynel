...
- Own curated, offline documentation exposed by `spynel docs` and concise help-topic metadata used by commands.
...
- Never read workspace state, environment values, histories, credentials, or arbitrary files; content is compiled from `content.go`.
...
- Keep notify guidance on the concrete `--workdir`, exact `--origin`, and `--message` form; do not teach destination placeholders or stdin composition.
- Do not present retired task/goal workflows, semantic heartbeats, or notification agents as current product behavior.
