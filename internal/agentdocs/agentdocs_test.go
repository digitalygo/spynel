...
func TestChannelsTopicDocumentsTelegramTopicsAndRichText(t *testing.T) {
	...
	for _, want := range []string{
		...
		"retry at most twice",
		"one-second then two-second",
		"three-minute delivery budget",
		"transcript echoes, welcome messages, and proactive text",
		"can arrive twice",
		"spynel/runtime/telegram-replies",
		"initial send plus five scheduled retries",
		"30 seconds, one minute, two, four, and eight minutes",
		"one-hour expiry",
		"acknowledged-chunk cursor",
		"no exactly-once guarantee",
		"never backfills a reply whose send failed before the durable queue existed",
		"attachments are not queued",
		"completed job does not prove Telegram delivery",
		"content-free",
		"--extension",
		"before_agent_start",
		"spynel_telegram",
