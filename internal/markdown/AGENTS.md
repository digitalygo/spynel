...
- `TelegramPlainChunks` chunks literal user content without Markdown interpretation: the input is HTML-escaped so provider HTML parsing cannot treat it as markup, then split under the same parsed-visible chunk and reply budget, entity, grapheme, and truncation rules as `TelegramChunks`, with over-budget output ending in `TelegramTruncationMarker`. `TelegramChunkPlainText` recovers visible content exactly up to the ordinary truncation boundary, and empty input stays a no-send.
