...
- Exactly one elected primary process runs the remote channels and background services, and every local interface reaches it over an authenticated loopback connection. Do not edit `.spynel/runtime/`; Spynel owns election records, harness session mappings, logs, outbox state, queued Telegram terminal replies, and transient runtime files.
- Do not edit or publish `.spynel/history/`, `.spynel/jobs/`, `.spynel/attachments/`, `.spynel/runtime/telegram-replies/`, `whatsapp.db`, or harness session data; they may contain secrets, captured execution output, remote media, and private conversations. Queued Telegram reply records are private but plaintext, and Spynel deletes them after confirmed delivery or one-hour expiry while it is running.
...
- `runtime/` owns election records, harness session mappings, logs, outbox state, queued Telegram terminal replies, and transient bounded work files.
