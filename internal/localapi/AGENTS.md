...
- Bound readiness polling independently from long message streams, preserve only a categorized sanitized last condition, and expose stable foreign-environment and readiness-timeout errors without endpoints, tokens, or private paths.
...
- Carry global `RuntimeStatus.LiveJobs` in registration and shared-state snapshots so primary and attached TUIs refresh through ordinary polling.
